...
def add_message_templates(templates, email):
    """
    批量添加回复模板
...
def delete_message_template(template_id, email):
    """
    删除指定ID的回复模板
    ...
    Returns:
        int: 受影响的行数
    """
    query = "DELETE FROM message_template WHERE id = %s AND userInfo = %s"
    params = (template_id, email)
    return execute_update(query, params)


def delete_all_message_templates(email):
    """
    删除用户的所有回复模板
    ...
    Returns:
        int: 受影响的行数
    """
    query = "DELETE FROM message_template WHERE userInfo = %s"
    params = (email,)
    return execute_update(query, params)


def update_message_template(template_id, content, email, image_urls=None):
    """
    更新指定ID的回复模板内容
    ...
    if image_urls is not None:
        query = "UPDATE message_template SET content = %s, image_urls = %s WHERE id = %s AND userInfo = %s"
        params = (content, image_urls, template_id, email)
    else:
        query = "UPDATE message_template SET content = %s WHERE id = %s AND userInfo = %s"
...
def main_handler(event, context):
    ...
    try:
        ...
        if action == 'add':
            ...
            affected_rows = add_message_templates(templates, email)
            return {
                "code": 0 if affected_rows > 0 else 1,
            ...
            affected_rows = delete_message_template(template_id, email)
            return {
                "code": 0 if affected_rows > 0 else 1,
            ...
            # 删除用户的所有模板
            affected_rows = delete_all_message_templates(email)
            return {
                "code": 0,
                "message": "success",
                "data": {
            ...
            affected_rows = update_message_template(template_id, content, email, image_urls)
            return {
                "code": 0 if affected_rows > 0 else 1,
